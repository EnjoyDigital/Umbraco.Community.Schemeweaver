...
Setting `EmitBreadcrumbsInDeliveryApi` to `false` removes the breadcrumb from Delivery API output in both modes. Under graph output the `BreadcrumbList` node is left out of the `@graph` and the `WebPage` node's `breadcrumb` reference goes with it, so nothing points at a missing node. In legacy mode the separate `BreadcrumbList` string is dropped. The tag helper and the backoffice JSON-LD preview are unaffected either way.
...
| `EmitBreadcrumbsInDeliveryApi` | `true` | Whether `BreadcrumbList` is included in the Delivery API `/json-ld` output and the `schemaOrg` index field. Applies in both modes: under graph output the breadcrumb node and the `WebPage` reference to it are omitted; in legacy mode the separate `BreadcrumbList` string is dropped. The server-rendered tag helper and the backoffice JSON-LD preview always emit breadcrumbs. |
