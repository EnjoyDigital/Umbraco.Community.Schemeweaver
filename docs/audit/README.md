...
3. Runs every node through `ISchemaValidator`: twenty-seven tier-1 Google Rich Results rule-sets plus a generic `@id` / `name` sanity check.
...
Twenty-seven Schema.org type families get specific Google Rich Results rules (required → Critical, recommended → Warning):
