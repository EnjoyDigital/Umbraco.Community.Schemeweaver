...
| **Umbraco Deploy OnPrem or Cloud** | A **paid, separately installed** product (included with Umbraco Cloud). The addon references only the freely available `Umbraco.Deploy.Infrastructure` contracts — if Deploy itself is not installed, the addon logs one warning at startup and stays inactive; it never breaks the site. |
...
- **Mapping**: content type alias + key, Schema.org type, enabled/inherited flags, `@id` override
...
On every schema deployment the source's mappings win, exactly like a uSync import. Note in particular: **`IsEnabled` travels in the artifact** — a mapping disabled directly on production will be re-enabled by the next deployment from an environment where it is enabled. Make mapping changes at the source of your deployment flow.
