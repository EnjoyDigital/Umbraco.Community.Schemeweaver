...
Returns all available Schema.org types, or a filtered subset when a search query is provided. Types are sourced from the Schema.NET.Pending assembly at startup.
...
These endpoints are only available when the `Umbraco.Community.SchemeWeaver.AI` satellite package is installed. If the package is not installed, all endpoints under `/ai/` return `404`.

### Base URL

```
/umbraco/management/api/v1/schemeweaver/ai
```
...
Returns whether the AI satellite package is installed and available. Use this endpoint to detect AI availability before showing AI features in the UI.
...
If this endpoint returns `404`, the AI package is not installed.

---

### POST /ai/suggest-schema-type/{contentTypeAlias}

Uses AI to suggest which Schema.org types best fit a given content type. Returns up to 3 ranked suggestions.
...
[
  {
    ...
    "confidence": 95,
    "reasoning": "Content type has title, bodyText, author, and publishDate properties typical of blog posts"
  },
  {
    "schemaTypeName": "Article",
    "confidence": 80,
    "reasoning": "Generic article type also fits but BlogPosting is more specific"
  }
]
```
...
```typescript
interface SchemaTypeSuggestion {
  schemaTypeName: string;
  confidence: number;
...
### POST /ai/suggest-schema-types-bulk

Analyses all non-element content types in a single batch and suggests Schema.org types for each.
    ...
    "suggestions": [
      {
        "schemaTypeName": "BlogPosting",
        "confidence": 90,
        "reasoning": "Has title, body, author, and date properties"
      }
    ]
  },
  {
    "contentTypeAlias": "productPage",
    "contentTypeName": "Product Page",
    "suggestions": [
      {
        "schemaTypeName": "Product",
        "confidence": 85,
        "reasoning": "Has price, sku, and description properties"
      }
    ]
  }
]
...
```typescript
interface BulkSchemaTypeSuggestion {
  contentTypeAlias: string;
...
|---|---|
| `200 OK` | Suggestions returned |
| `500 Internal Server Error` | AI bulk analysis failed |
...
| Status | Description |
|---|---|
| `200 OK` | Suggestions returned (heuristic fallback on AI failure) |
| `500 Internal Server Error` | Both AI and heuristic mapping failed |
