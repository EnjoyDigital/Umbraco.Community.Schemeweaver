...
- **Cross-node sources, nested-block routes, multiple targets.** Schema properties the page cannot supply itself (a post's `publisher`, a department's `location`) are offered the real properties of the content types above and beside it, discovered from declared structure and/or sampled pages, and become `parent`, `ancestor` or `sibling` rows that name their source type. Block Lists with several block types, or blocks inside blocks, are emitted as per-block-type `routes`. A bind answer's strong runner-up becomes a second, un-ticked target offered for review (`title` to both `headline` and `name`). Sample values stay on your site unless you opt in, and even then only a published node is ever read.
