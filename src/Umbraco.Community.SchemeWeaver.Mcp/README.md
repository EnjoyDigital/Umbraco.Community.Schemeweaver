...
| `save-schema-mapping` | Create/replace a mapping (full property-mapping model: property, static, parent, ancestor, sibling, blockContent, reference). `blockContent` uses a nestable `routes` resolverConfig that maps blocks (including blocks nested inside blocks and Block Grid areas) to nested Schema.org objects |
